...
def main():
    ...
    # Verify that files actually exist (some cities might not have sen2spring)
    print(f"Verifying file existence for {len(df)} paths...")
    file_exists = []
    ...
    df = df[pd.Series(file_exists)]
    print(f"After file verification: {len(df)} files exist")
